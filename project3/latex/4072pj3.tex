...
\renewcommand{\baselinestretch}{1.025} %設定行距1.0
...
\begin{document}
    ...
    {\begin{center} %以下文字置中
        ...
        \hspace*{\fill} \\
        \bf \thirty {平面多連桿機構套件之合成與應用}\\
        {\renewcommand\baselinestretch{1.3}\selectfont %設定以下行距
        \bf \thirty {Synthesis and Application of Planar Multi-link Mechanism Package}\\}
    \end{center}}
    \par} %結束指定行距
...
%專題製作合可證明
 ...
 {\begin{center}
    ...
    \noindent \fourteen 專題製作題目\enspace :\quad 平面多連桿機構套件之合成與應用
    \hspace*{\fill} \\
    \hspace*{\fill} \\
...
%摘要
    ...
    \end{center}
	\hspace*{\fill} \\
	{\renewcommand\baselinestretch{1.025}\selectfont %設定以下行距
...
\pagenumbering{arabic} %設定以下頁碼為阿拉伯數字
	...
	\section{研究目的與方法}
      ...
      \begin{figure}[hbt!]
        \centerline{
        \includegraphics[width=6in, height=3in]{架構圖.png}}
        \caption{研究架構圖}
        \label{架構圖:scale}
      \end{figure}
...
\newpage
        ...
        \begin{figure}[H]
        \centering
        \includegraphics[scale=0.65]{GIM.png} 
        \caption{GIM合成介面} 
        \label{fig_GIM:scale}
    	\end{figure}
\newpage
...
\section{Pyslvs-UI 範例}
     ...
     \begin{figure}[H]
        \centering
        \includegraphics[scale=0.4]{曲柄滑塊機構.png} 
        %照片須放在和文檔相同之資料夾內
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[scale=0.3]{史蒂芬生機構範例.jpg} 
        %照片須放在和文檔相同之資料夾內
...
%--------------避震機構分析範例--------------%	
	   \newpage
       \section{避震機構合成結果}
       \fourteen {此小節會以Pivot Cycles Firebird 29(雙連桿懸架類型)作為分析範例利用4.1節所推導出的適應性函數配合4.2節介紹的評比標準進行討論以檢驗得到的尺寸優劣。圖\ref{fig_Firebird_mechanism:scale}為Firebird在Pyslvs-UI中的機構簡圖。}
		\begin{figure}[hbt!]
		%----figure1----%
		\includegraphics[scale=0.4]{synthesis_result_bike2.png}
        \caption{Pivot Cycles Firebird 29}
        \label{Pivot cycles firebird:scale}
        %----figure1----%
		%----figure2----%
        \centering
    	...
    	\newpage
    	\fourteen {表$\ref{tab_尺寸合成結果:scale}$為Pivot Cycles Firebird 29實際產品的後避震機構的旋轉接頭座標點位置和本團隊利用差分進化演算法及藉由避震機構評量推導的適應性函數求得的各旋轉接頭位置。\ref{compare:scale}為實際產品的尺寸和本團隊尺寸合成所得到的尺寸在Anti-squat評比中所得到的結果比較，由圖中可得到在一開始的時候會有些差距但是當後輪行程(wheel travel)到60mm的時候兩條曲線會慢慢貼合。}\\
		...
		%htbp代表表格浮動位置
			...
			\caption{尺寸合成結果}%表:標題
			\large%字體大小
			\label{tab_尺寸合成結果:scale}
			\begin{tabular}{|c|c|c|c|c|c|}
			\hline
			\diagbox[width=9em] & p0 & p1 & p2 & p3 & p4 \\ 
			\hline
			實際產品位置 	&(0, 0)&(-41.5, -5.1)&(-397.5, -42.5)&(18, 112)&(-3, 155)\\
			\hline
			尺寸合成位置 	&(0, 0)&(-40, -40)&(-390, -40)&(20m 120)&(-5, 150)\\
			\hline
			\end{tabular}
		\end{table}    	
		...
		\begin{figure}[hbt!]
		\centering
		\includegraphics[scale=0.8]{compare.png}
        \caption{尺寸合成結果與實際產品比較}
        \label{compare:scale}
    	\end{figure} 
    	%----figure----%   		
...
%--------------3D零件自動生成--------------%
	...
	\end{enumerate}    
      ...
      \subsection{xml樹狀結構}
      {\begin{flushleft}
      \quad \quad \fourteen {xml的結構像樹狀般，一層包著一層，由一個本體延伸出許多分支向外擴展，又被稱為xml樹。在CoppeliaSim裡是由許多tag(標籤)組合而成，不管是本體還是分支都是用tag來呈現，在CoppeliaSim裡最外層的tag就代表一個物件，可以從中去設定物件的參數，像是CoppeliaSim內建的物體、馬達或是從外部匯入的stl檔都可以使用xml去調整參數，複雜的從屬關係也可以從xml調整，如圖\ref{fig_xml_tree_structure_joint:scale}、\ref{fig_xml_tree_structure_shape:scale}所示。}
      \end{flushleft}
      \hspace*{\fill} \\
      \hspace*{\fill} \\
      \hspace*{\fill} \\
      ...
      \begin{figure}[hbt!]
        \centering
        \includegraphics[scale=0.6]{xml_tree_structure_joint.PNG} 
        \caption{馬達參數}
        \label{fig_xml_tree_structure_joint:scale}
      \end{figure}
...
print('finish entire code')
        ...
        \chapter{未來研究建議}
        {\begin{flushleft}
        \fourteen {本專題已建立一流程從機構尺寸合成、機構量化評比、動態系統設定、運動模擬，後續可將原有的動態系統加入自行車重量、騎乘者的重量、各接觸點的反力......以建立更完整的動態系統，接著可利用有限元素法進行結構分析。}
        \begin{figure}[hbt!]
        \centering
  ...
  %---------------附錄-------------% 
	...
	\end{center}
	 ...
	 \begin{itemize} 
        \item LaTex 簡介：LaTex 是一套由美國計算機科學家 Leslie Lamport 於 20 世紀 80 年代初期所開發的排版格式，相較於 Word 的「所見所得」編輯模式，純文字檔的模式更像在寫程式。其最初創建的宗旨，就是要能完美排版文件，它除了可編輯中文 (簡、繁體)、英文， 更可以畫圖、製表，甚至繪製樂譜和符號，其強大的對齊功能與多樣化的工具，使LaTex 已變成編輯各式文件的最佳排版方法。 
		\item 特點：\\
		\end{itemize} 
	...
	\quad \fourteen {德國詩人歌德曾說過：「我不應把我的作品全歸功於自己的智慧，還應歸功於我以外向我提供素材的成千成萬的事情和人物。」}

	\quad \fourteen {本研究之所以可以順利完成，首先感謝負責指導的專題教授嚴家銘老師，老師細心的指導與適時的提供建議下，確立整體研究架構與目標，更讓我們深入了解平面多連桿機構套件的理論，並在研究過程中培養我們解決問題以及獨立思考的能力。}

	\quad \fourteen {再來要感謝共同指導教授李武鉦老師，運用課堂所教導的Python程式語法，使我們在專題研究中，得以延伸學習與應用，跳脫僵化的思維去探討所要面對之問題。}
	\quad \fourteen {最後感謝學校提供完善的設備與資源，使本專題能夠圓滿完成；在專題研究上，系上提供軟硬體設備以及專題實驗室，讓我們可以利用空堂及課後時間製作與討論專題。}
    ...
    \selectfont %設定以下行距
    {\begin{textblock}{15}(3.5,1.8) %{寬度}(以左上角為原點之右移量,下移量)
\noindent\fourteen \makebox[4em][s]{姓名}\enspace:\enspace
...
\fourteen \makebox[4em][s]{經歷}\enspace:\enspace
    ...
    \end{textblock}}
    {\renewcommand\baselinestretch{0.96}\selectfont %設定以下行距
    {\begin{textblock}{15}(3.5,3) %{寬度}(以左上角為原點之右移量,下移量)
\noindent\fourteen \makebox[4em][s]{姓名}\enspace:\enspace%\noindent指定首行不進行縮排
...
\fourteen \makebox[4em][s]{經歷}\enspace:\enspace
    ...
    {\begin{textblock}{6}(0,4.25)
    \begin{figure}
        \includegraphics[width=1.15in]{st4.jpg} %{}內是圖片文件的相對路徑
    \end{figure}
    \end{textblock}}
    {\renewcommand\baselinestretch{0.96}\selectfont %設定以下行距
    {\begin{textblock}{15}(3.5,4.25) %{寬度}(以左上角為原點之右移量,下移量)
\noindent\fourteen \makebox[4em][s]{姓名}\enspace:\enspace
\fourteen \makebox[4em][s]{吳佳穎}\\ 
\fourteen \makebox[4em][s]{學號}\enspace:\enspace
\fourteen \makebox[4em][s]{40723153} \\ 
\fourteen \makebox[4em][s]{畢業學校}\enspace:\enspace
\fourteen \makebox[9em][s]{國立虎尾科技大學}\\
\fourteen \makebox[5em][s]{\quad}\enspace\enspace
\fourteen \makebox[8em][s]{機械設計工程系}\\
\fourteen \makebox[4em][s]{經歷}\enspace:\enspace
    ...
    {\begin{textblock}{6}(0,5.7)
    \begin{figure}
        \includegraphics[width=1.15in]{st5.jpg} %{}內是圖片文件的相對路徑
    \end{figure}
    \end{textblock}}
    \renewcommand\baselinestretch{0.96}\selectfont %設定以下行距
    {\begin{textblock}{15}(3.5,5.7) %{寬度}(以左上角為原點之右移量,下移量)
	\noindent\fourteen \makebox[4em][s]{姓名}\enspace:\enspace
	\fourteen \makebox[4em][s]{蔡育灃}\\ 
	\fourteen \makebox[4em][s]{學號}\enspace:\enspace
	\fourteen \makebox[4em][s]{40723245} \\ 
	\fourteen \makebox[4em][s]{畢業學校}\enspace:\enspace
	\fourteen \makebox[9em][s]{國立虎尾科技大學}\\
	\fourteen \makebox[5em][s]{\quad}\enspace\enspace
	\fourteen \makebox[8em][s]{機械設計工程系}\\
...
\vspace{4em}
\fontsize{24pt}{24pt}\selectfont 【11】 \\
...
\hspace{-0.5em}：\\
\vspace{0.5em}
\rotatebox[origin=cc]{270}{\sectionef\LARGE
\textbf{109-4-CAD-3006-1}}\\ %旋轉
\vspace{1em}
\rotatebox[origin=cc]{270}{\sectionef\LARGE
\textbf{}}\\
\vspace{0.5em}
